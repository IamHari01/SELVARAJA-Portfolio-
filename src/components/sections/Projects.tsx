      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-24 gap-8">
          <div>
            ...
            >
              PROJECT <span className="text-primary">ARCHITECTURE</span>
            </motion.h2>
          </div>
          <motion.div 
            initial={{ opacity: 0 }}
                ...
                <div className={idx % 2 === 0 ? "order-2 lg:order-1" : "order-2"}>
                  <div className="glass-panel p-8 md:p-14 rounded-[2.5rem] relative overflow-hidden fresnel-glow">
                    <div className="absolute top-0 right-0 p-6 opacity-5 md:opacity-20">
                      <Code2 className="text-white w-12 h-12 md:w-16 md:h-16" />
                    </div>
                    
                    <div className="flex flex-wrap gap-2.5 mb-8">
                      {project.tech.map(t => (
                        <Badge key={t} variant="secondary" className="bg-white/5 border-white/10 text-[9px] uppercase font-bold tracking-[0.2em] px-3 py-1">
                          {t}
                        </Badge>
                      ))}
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
                      {project.features.map(f => (
                        <div key={f} className="flex items-center gap-4 text-xs md:text-sm text-white/40">
                          <Workflow className="w-5 h-5 text-primary shrink-0 opacity-60" />
                          <span>{f}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-5">
                      ...
                      <button className="flex-1 flex items-center justify-center gap-3 px-8 py-4 rounded-full bg-primary/10 text-primary hover:bg-primary/20 transition-all font-bold text-xs uppercase tracking-widest border border-primary/20">
                  ...
                  >
                    ...
                    <div className="absolute inset-0 z-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-700">
                      <div className="p-5 bg-black/40 backdrop-blur-2xl rounded-full border border-white/20 scale-75 group-hover:scale-100 transition-transform duration-700">
                        <Workflow className="w-12 h-12 text-primary animate-pulse" />
                      </div>
                    </div>
                  </motion.div>
                </div>
